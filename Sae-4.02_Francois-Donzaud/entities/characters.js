...
AFRAME.registerComponent('play-sound-on-animation-complete', {
    init: function () {
        this.el.addEventListener('animationcomplete', (event) => {
            // Vérifiez si l'animation terminée est 'animation__walk'
            if (event.detail.name === "animation__walk") {
                console.log('Animation walk complete'); // Vérifiez si l'événement est capturé
...
function checkAllItemsPresent(needed) {
    ...
    return Object.entries(needed).every(([key, value]) => {
        ...
        // FAIRE EN SORTE DE JOUER UN SON QUAND LE MAUVAIS OBJET EST DEPOSER

        let YesMusic = new Audio('./assets/sounds/Score_Up_sound.mp3');
        YesMusic.volume = 0.3; // volume du score

        let NoMusic = new Audio('./assets/sounds/Buzzer.mp3');
        NoMusic.volume = 0.1; // volume du buzzer

        let wrongItemDetected = false;

        if (depositedItem) {
            console.log("Deposited Item:", depositedItem.value);
            if (depositedItem.count === value) {
            if (depositedItem.value === needed[0]) {
                if (!wrongItemDetected) {
                NoMusic.play();
                wrongItemDetected = true;
                }
            } 
            else if (depositedItem.value !== needed[0]) {
                YesMusic.play();
                return true;
            }
            } 
